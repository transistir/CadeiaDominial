...
Services are organized by domain in `/dominial/services/`:

```
services/
├── Hierarchy & Chain Services (7 files)
├── Transaction Services (12 files)
├── Document Services (3 files)
├── Supporting Services (6 files)
└── Backup/Archive Files (3 files - legacy)
```

Total: **30+ service files** with ~7,000 lines of business logic
...
**File:** `dominial/services/hierarquia_arvore_service.py` (351 lines)

**Purpose:** Builds tree structure for D3.js visualization.
...
**Data Structure (JSON for D3):**
```json
{
  "id": "doc_123",
  "tipo": "matricula",
  "numero": "12345",
  "cartorio": "1º CRI Salvador",
  "data": "2020-01-15",
  "nivel": 0,
  "children": [
    {
...
# Build tree for D3 visualization
arvore = HierarquiaArvoreService.construir_arvore_cadeia_dominial(
    documento_id=123,
    origens_escolhidas={'456': 2}  # Optional: user-selected origins
)
```

**Related Files:**
- `hierarquia_arvore_service_backup.py` - Backup of previous version
- `hierarquia_arvore_service_corrigido.py` - Corrected version (legacy)
- `hierarquia_arvore_service_melhorado.py` - Improved version (legacy)
