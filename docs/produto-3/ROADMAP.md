...
> exportação é **regra decidida pelo produto**, e um documento alcançado por
> vários galhos sai **uma única vez** na exportação, no **primeiro** galho que
> o alcança — nunca repetido (ex.: no imóvel 384 a `T2391` é citada por vários
> documentos, entre eles `T3281` e `T3280`). A regra e a ordem-alvo acima foram
> definidas pelo dono do produto; o estado atual (ordem plana) foi medido com
> dados reais dos imóveis 114 (17 docs, test) e 384 (30 docs, test). Não
...
1b. **#201 Fase 0b — ordenação canônica da cadeia** 🔨 **PR #205 ABERTO**
   ...
   - Chave única em `dominial/utils/ordenacao_cadeia.py`, consumida por
     ...
     do tronco.
   - Achados **M-1..M-3 + M-6 + N-1 + M-7 corrigidos** nos commits
     `95666ec7..2877a510` (3 commits —
     https://github.com/transistir/CadeiaDominial/compare/95666ec7...2877a510);
     **62 testes focados passam** (`test_issue_201_origens_cadeia_tabela.py` +
     ...
     (M-1, M-2, M-3), corrigidos nos commits seguintes; M-6 e N-1 vêm da
     mensagem do commit `1c0db87c`, M-7 da mensagem do `2877a510`. Não há
     registro público de rodadas posteriores de review no PR.
   ...
   - **Evidência única das exportações**, medida pelo orquestrador na
     ...
     do sort plano, não de grupos: a topologia real —
     `M2622 → T9231 → T13963 → T9001`, conferível no campo **Origem** de cada
     documento (exibido no card do documento na tela `ver-cadeia-dominial/`)
     — coloca `T9231` antes de `T13963`, mas a saída atual inverte essa ordem.
